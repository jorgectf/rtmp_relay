...
namespace relay
{
    PullReceiver::PullReceiver(cppsocket::Network& aNetwork,
                               Socket& aSocket,
                               float aPingInterval,
        ...
        applicationDescriptors(aApplicationDescriptors)
    {
        ...
        socket.setReadCallback(std::bind(&PullReceiver::handleRead, this, std::placeholders::_1, std::placeholders::_2));
        socket.setCloseCallback(std::bind(&PullReceiver::handleClose, this, std::placeholders::_1));
        socket.startRead();
    }

    void PullReceiver::reset()
    {
        socket.close();
        data.clear();
    ...
    void PullReceiver::update(float delta)
    {
    ...
    bool PullReceiver::getPacket(std::vector<uint8_t>& packet)
    {
        if (data.size())
        {
            packet = data;
    ...
    void PullReceiver::handleRead(cppsocket::Socket&, const std::vector<uint8_t>& newData)
    {
    ...
    void PullReceiver::handleClose(cppsocket::Socket&)
    {
    ...
    bool PullReceiver::handlePacket(const rtmp::Packet& packet)
    {
    ...
    void PullReceiver::sendServerBandwidth()
    {
        rtmp::Packet packet;
    ...
    void PullReceiver::sendClientBandwidth()
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::NETWORK;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendSetChunkSize()
    {
        rtmp::Packet packet;
    ...
    void PullReceiver::sendConnectResult(double transactionId)
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendCheckBWResult(double transactionId)
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendCreateStreamResult(double transactionId)
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendReleaseStreamResult(double transactionId)
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendOnFCPublish()
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    void PullReceiver::sendPublishStatus(double transactionId)
    {
        rtmp::Packet packet;
        packet.channel = rtmp::Channel::SYSTEM;
        packet.timestamp = 0;
    ...
    bool PullReceiver::connect(const std::string& applicationName)
    {
    ...
    void PullReceiver::getInfo(std::string& str, ReportType reportType) const
    {
        switch (reportType)
        {
            ...
            case ReportType::HTML:
            {
                str += "<h2>Receiver " + std::to_string(id) + "</h2>" +
                    ...
                    "<tr><td>" + streamName + "</td><td>" + (socket.isReady() ? "Connected" : "Not connected") + "</td><td>" + ipToString(socket.getRemoteIPAddress()) + ":" + std::to_string(socket.getRemotePort()) + "</td><td>";
